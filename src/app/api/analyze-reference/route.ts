...
export async function POST(req: NextRequest) {
  ...
  const tempPath = join(tempDir, `${Date.now()}-ref${ext}`);
  const buffer = Buffer.from(await file.arrayBuffer());
  await writeFile(tempPath, buffer);

  try {
    const result = await runReferenceAnalysis(tempPath);
...
function runReferenceAnalysis(videoPath: string) {
  return new Promise<unknown>((resolve, reject) => {
    const python = process.env.PYTHON_PATH || "python";
    const scriptPath = join(process.cwd(), "scripts", "analyze_reference.py");
    ...
    proc.on("close", (code) => {
      if (code !== 0) {
        reject(new Error(`Reference analysis failed (code ${code}): ${stderr.slice(-500)}`));
        return;
      }
      try {
